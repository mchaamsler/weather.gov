...
export default async () => {
  const metadata = document.querySelector("weathergov-location-metadata");
  const geometry = JSON.parse(metadata.getAttribute("geometry"));
  const point = geometry[0];

  const map = leaflet
    .map("weathergov_location_radar")
    .setView([point.lat, point.lon], 9);
  ...
  leaflet.marker([point.lat, point.lon], { icon: locationIcon }).addTo(map);
